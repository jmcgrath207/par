...
func Start(managerIP string) {
	proxyMutex.Lock()
	if store.ProxyInit == 1 {
		return
	}
	namespace, _ = os.ReadFile("/var/run/secrets/kubernetes.io/serviceaccount/namespace")
	k8sClient = client.NewNamespacedClient(store.ClientK8s, string(namespace))
	GetProxyServiceIP()
	RenderProxyConfig(managerIP)
	store.ProxyInit = 1
	store.ProxyWaitGroup.Done()
...
func setProxyPodsClientId(replicas int) {
	...
	for {
		...
		for _, pod := range podList.Items {
			...
			store.ClientId[pod.Status.PodIP] = "1"
			count = count + 1
		}
		if count == podCount {
			break
